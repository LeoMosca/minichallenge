    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.row == 0){
            if let cell = tableView.dequeueReusableCell(withIdentifier: "ToolCellHeader", for: indexPath) as? ToolsHeaderTableViewCell{
                ...
                if(lang.topics?[topicIndex].tools?[toolIndex].content?.count == 1){
                    cell.numberOfItens.text = String(lang.topics?[topicIndex].tools?[toolIndex].content?.count ?? 0) + " Item"
                }
                else{
                    cell.numberOfItens.text = String(lang.topics?[topicIndex].tools?[toolIndex].content?.count ?? 0) + " Itens"
                }
                cell.toolName.text = lang.topics?[topicIndex].tools?[toolIndex].name
                //cell.toolImage. = lang.topics?[topicIndex].tools?[toolIndex].content?[0].image
                cell.toolDescription.text = lang.topics?[topicIndex].tools?[toolIndex].description
                return cell
